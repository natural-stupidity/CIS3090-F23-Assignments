...
int main(int argc, char *argv[]) {
    ...
    execTime = (end.tv_sec - start.tv_sec);
...
void* renderChunk(void* params) {
	...
	for (int y = data->startY; y < data->endY; y++)
	{
		for (int x = data->startX; x < data->endX; x++) {
            ...
            data->img[(x + y*WIDTH)*3 + 0] = (unsigned char)min(red*255.0f, 255.0f);
            data->img[(x + y*WIDTH)*3 + 1] = (unsigned char)min(green*255.0f, 255.0f);
            data->img[(x + y*WIDTH)*3 + 2] = (unsigned char)min(blue*255.0f, 255.0f);
        }
	}
}
